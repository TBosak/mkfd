...
if (import.meta.main) {
	...
	class ImapWatcher {
		 ...
		 * without destroying the old socket, so sockets and listeners
		 * accumulated until the provider refused new connections (issue #77).
		 * The lifecycle tears the previous one down before calling this.
		 */
		 ...
		 * lifecycle sees the resulting `close` and retries — which is what the
		 * old code did by having `openBox` inside the same try as `connect`.
		 */
		...
		public stop(): void {
			...
			// Terminal: cancels any pending reconnect as well as closing the
			// socket. The old stop() ended the socket but left the reconnect
			// timer running, so a stopped watcher reconnected anyway.
			this.lifecycle.stop();
		}
	}
