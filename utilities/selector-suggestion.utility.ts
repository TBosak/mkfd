 ...
 */
function _findMainContentArea($: CheerioAPI): Cheerio<AnyNode> {
	...
	$("div, section, article, main").each((_, el) => {
		...
		if (
			/\bcontent\b|\bmain\b|\barticle\b|\bpost\b|\bentry\b|\bstory\b|\bnews\b|\bfeed\b/.test(
				combined,
			)
		) {
			score += 25;
		...
		if (
			/\bnav\b|\bsidebar\b|\baside\b|\bfooter\b|\bheader\b|\bmenu\b|\bcomment\b|\bad\b|\bbanner\b/.test(
				combined,
			)
		) {
			score -= 50;
...
function chooseBestIterator(
	...
	for (const candidate of candidates) {
		...
		const linkDensity =
			avgTextLength > 0 ? (avgLinkCount / avgTextLength) * 1000 : 0;
	...
	for (let i = 0; i < Math.min(3, scores.length); i++) {
		...
		console.log(
			`     Coverage: title=${(s.coverage.title * 100).toFixed(0)}% link=${(s.coverage.link * 100).toFixed(0)}% desc=${(s.coverage.description * 100).toFixed(0)}% author=${(s.coverage.author * 100).toFixed(0)}% date=${(s.coverage.date * 100).toFixed(0)}%`,
		);
		console.log(
			`     Penalties: linkDensity=${s.penalties.linkDensity.toFixed(1)} boilerplate=${s.penalties.boilerplate.toFixed(1)} shallow=${s.penalties.tooShallow.toFixed(1)} overlap=${s.penalties.overlap.toFixed(1)} size=${s.penalties.size.toFixed(1)}`,
		);
	}
...
export function suggestSelectorsFromHtml(
	...
	const parentBased = findCommonParents(
		$content,
		Object.values(fieldCandidates),
	);
	const allCandidates = [
		...new Set([...structuralIterators, ...contentIterators, ...parentBased]),
	];
	...
	const iteratorCandidates = chooseBestIterator(
		$content,
		allCandidates,
		fieldCandidates,
		12,
	...
	for (const candidate of iteratorCandidates) {
		...
		items.each((_, item) => {
			...
			// Check for link
			const linkMatch = $item
				.find(tempChildSelectors.link.selector ?? "")
				.first();
			...
			const descMatch = $item
				.find(tempChildSelectors.description.selector ?? "")
				.first();
		...
		const minRequiredLinks = Math.max(
			2,
			Math.min(5, Math.ceil(items.length * 0.4)),
		);

		// Also prefer iterators where at least 30% of items have descriptions (filters nav)
		const descriptionCoverage =
			items.length > 0 ? itemsWithDescription / items.length : 0;
		...
		if (uniqueLinks.size >= minRequiredLinks && hasGoodDescriptionCoverage) {
			...
			console.log(
				`[Iterator Validation] Selected "${candidate}" with ${uniqueLinks.size} unique links, ${(descriptionCoverage * 100).toFixed(0)}% description coverage`,
			);
			...
			items.slice(0, 5).each((_, item) => {
				...
				const enclosureElem = $item
					.find(tempChildSelectors.enclosure.selector ?? "")
					.first();
			...
			console.log(
				`[Link Detection] ${relativeLinksCount} relative, ${absoluteLinksCount} absolute → isRelative: ${isLinkRelative}`,
			);
			if (relativeEnclosuresCount + absoluteEnclosuresCount > 0) {
				console.log(
					`[Enclosure Detection] ${relativeEnclosuresCount} relative, ${absoluteEnclosuresCount} absolute → isRelative: ${isEnclosureRelative}`,
				);
			}
			...
			if (uniqueLinks.size < minRequiredLinks) {
				reasons.push(
					`${uniqueLinks.size} unique links (need ${minRequiredLinks})`,
				);
			}
			if (!hasGoodDescriptionCoverage) {
				reasons.push(
					`${(descriptionCoverage * 100).toFixed(0)}% description coverage (need 30%)`,
				);
			}
			console.log(
				`[Iterator Validation] Rejected "${candidate}" - ${reasons.join(", ")}`,
			);
		}
	}
	...
	if (!iteratorSelector && iteratorCandidates.length > 0) {
		console.log(
			"[Iterator Validation] No iterator passed validation, using first candidate as fallback",
		);
		iteratorSelector = iteratorCandidates[0];
		childSelectors = suggestChildSelectors(
			$content,
			iteratorSelector,
			fieldCandidates,
		);
	...
	console.log(
		`[Selector Suggestion] Returning selectors for iterator: ${iteratorSelector}`,
	);
 ...
 */
function titleLinkAffinity($item: Cheerio<AnyNode>, candidate: string): number {
	...
	return (
		(hasAnchorChild ? 20 : 0) + (isAnchor ? 30 : 0) + (isInsideAnchor ? 15 : 0)
	);
}
 ...
 */
function _stableSelectorFromElement($el: Cheerio<AnyNode>): string {
...
function suggestChildSelectors(
	...
	if (isTableMode) {
		console.log(
			"[Table Mode] Detected table-based iterator, using table-specific heuristics",
		);
	...
	for (const field of Object.keys(fieldCandidates)) {
		...
		for (const candidate of fieldCandidates[field]) {
			...
			const uniqueRatio =
				nonEmptyCount > 0 ? uniqueValues.size / nonEmptyCount : 0;
			...
			if (field === "title") {
				...
				const hasDateTokens = values.some(
					(v) =>
						/\b\d{1,4}\b/.test(v) ||
				...
				const hasAuthorContext =
					/author|byline|writer|contributor|posted-by/i.test(selectorLower);
				if (hasAuthorContext) {
					score += 40;
				}
				...
				elements.forEach((el) => {
					...
					if (
						/author|byline|writer|contributor/i.test(
			...
			const boilerplatePattern =
				/sign in|subscribe|read more|continue reading|home|menu/i;
			const boilerplateCount = values.filter((v) =>
				boilerplatePattern.test(v),
			).length;
		...
		if (candidateStats.length > 1) {
			...
			const closeScorers = candidateStats.filter(
				(c) => c.score >= topScore * 0.9,
			);
		...
		const isRelative = Boolean(attrValue && !/^https?:\/\//i.test(attrValue));
...
function isStableClassToken(token: string): boolean {
	...
	const digitSymbolRatio =
		(token.match(/[0-9_-]/g) || []).length / token.length;
	if (digitSymbolRatio > 0.4) return false;

	// Reject obvious Tailwind-like utilities (optional - can tune)
	const tailwindPatterns =
		/^(flex|grid|p-\d|m-\d|text-|bg-|border-|rounded-|shadow-|w-\d|h-\d)/;
...
function minimizeSelector(
	...
	const dataAttrs = Object.keys($el.attr() || {}).filter((k) =>
		k.startsWith("data-"),
	);
	...
	for (const candidate of candidates) {
		...
		const coverageThreshold =
			field === "author" ? 0.2 : field === "date" ? 0.5 : 0.7;
