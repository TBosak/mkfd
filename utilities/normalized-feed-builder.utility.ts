...
export function buildFeedFromNormalizedItems(opts: NormalizedFeedMeta): Feed {
	...
	const title =
		overrides.title ?? metadata.title ?? sourceFeedMeta.title ?? feedName;
	const description =
		overrides.description ??
		metadata.description ??
		sourceFeedMeta.description ??
		"";
	const link =
		metadata.link ??
		sourceFeedMeta.link ??
		serverUrl ??
		`https://example.com/feeds/${feedId}`;
	...
	for (const item of items) {
		...
		feed.addItem({
			...
			author:
				typeof item.author === "string" ? [{ name: item.author }] : item.author,
