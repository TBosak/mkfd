...
function encryptPendingProtectedValues(
	...
	const encrypted: ProtectedRecord = {};
	for (const [name, value] of Object.entries(record)) {
		if (isProtectedValue(value)) {
			encrypted[name] =
				value.type === "protected" && value.value !== "********"
					? protectValue(value.value, key)
					: value;
...
export function castFeedFormDataToFeedConfig(
	...
	const base = {
		...
		webhook:
			(data.webhook as any)?.enabled && (data.webhook as any)?.url
				...
				: undefined,
		flaresolverr:
			(data.flaresolverr as any)?.enabled &&
			(data.flaresolverr as any)?.serverUrl
				? {
						enabled: true,
	...
	if (feedType === "webScraping") {
		...
		const iterator =
			buildCSSTargetFromForm("item", d) ??
			new CSSTarget((data.itemSelector as string) ?? "");
		const extractionMode =
			(data.extractionMode as string | undefined) ??
			(data.extraction as any)?.mode;
		const extraction =
			extractionMode && extractionMode !== "cssSelectors"
				? {
						mode: extractionMode,
						mappings: {
							title: (data.jsonLdTitlePath as string) || "headline",
							description:
								(data.jsonLdDescriptionPath as string) || "description",
						...
						drillChain: Array.isArray((data.extraction as any)?.drillChain)
							? (data.extraction as any).drillChain
							: undefined,
					}
				: undefined;
				...
				request:
					data.requestMode === "form"
						? {
								...
								userAgentProfileId: data.userAgentProfileId as
									| string
									| undefined,
								userAgentOverride: data.userAgentOverride as string | undefined,
								method: (data.formMethod as any) ?? "GET",
								actionUrl:
									(data.formActionUrl as string) ||
									(data.feedUrl as string) ||
									"",
								encoding:
									(data.formEncoding as any) ??
	...
	if (feedType === "rest") {
		return {
			...
			config: {
				...
				params: encryptPendingProtectedValues(
					kvPairsToRecord(data.apiParams),
					encKey,
				),
				apiSpecificHeaders: encryptPendingProtectedValues(
					kvPairsToRecord(data.apiHeaders),
					encKey,
				),
	...
	if (feedType === "feedTransformer") {
		...
		return {
			...
			feedTransformer: {
				...
				mergeStrategy:
					(data.transformerMergeStrategy as any) ??
					(data.feedTransformer as any)?.mergeStrategy ??
					"dateDesc",
				maxItems:
					Number(
						data.transformerMaxItems ?? (data.feedTransformer as any)?.maxItems,
					) || undefined,
				dedupeAcrossSources:
					(data.transformerDedupeAcrossSources as boolean | undefined) ??
					(data.feedTransformer as any)?.dedupeAcrossSources ??
					true,
				...
				items: (data.feedTransformer as any)?.items ?? {
					guidStrategy:
						(data.transformerGuidStrategy as any) ?? "existingOrLinkHash",
					dateStrategy:
						(data.transformerDateStrategy as any) ??
					...
					link: {
						removeTrackingParams:
							data.transformerRemoveTrackingParams !== false,
	...
	if (feedType === "sitemap") {
		return {
			...
			sitemap: {
				...
				descriptionStrategy:
					(data.sitemapDescriptionStrategy as any) ?? "sitemapMetadata",
			},
	...
	if (feedType === "calendar") {
		return {
			...
			calendar: {
				...
				timezoneFallback:
					(data.calendarTimezoneFallback as string) || undefined,
	...
	if (feedType === "graphql") {
		return {
			...
			graphql: {
				endpoint:
					(data.graphqlEndpoint as string) ?? (data.feedUrl as string) ?? "",
				method: "POST",
				headers: encryptPendingProtectedValues(
					kvPairsToRecord(data.graphqlHeaders),
					encKey,
				),
	...
	if (feedType === "filesystem") {
		return {
			...
			filesystem: {
				...
				guidStrategy:
					(data.filesystemGuidStrategy as any) ?? "pathAndModifiedTime",
				titleStrategy: (data.filesystemTitleStrategy as any) ?? "filename",
				descriptionStrategy:
					(data.filesystemDescriptionStrategy as any) ?? "fileMetadata",
				sidecar: {
					enabled: Boolean(data.filesystemSidecarEnabled),
					extension: (data.filesystemSidecarExtension as string) || ".json",
				},
				extraction: {
					enabled: Boolean(data.filesystemExtractionEnabled),
					maxCharacters: Number(data.filesystemExtractionMaxCharacters) || 400,
					maxFileSizeBytes:
						Number(data.filesystemExtractionMaxFileSizeBytes) || 262144,
					supportedExtensions: splitList(data.filesystemExtractionExtensions, [
						"txt",
						"md",
						"html",
					]),
	...
	if (feedType === "webhook") {
		...
		return {
			...
			webhookFeed: {
				...
				dateStrategy:
					(data.webhookDateStrategy as any) ?? "payloadDateOrReceivedAt",
	...
	if (feedType === "serviceConnector") {
		...
		return {
			...
			serviceConnector: {
				service:
					(data.serviceConnectorService as string) ??
					(data.serviceConnector as any)?.service ??
					"jellyfin",
				connection: {
					label: (data.serviceConnectorLabel as string) || undefined,
					settings: {
						serverUrl:
							(data.serviceConnectorServerUrl as string) ??
							(data.serviceConnector as any)?.connection?.settings?.serverUrl ??
							"",
					},
					auth: {
						mode: "apiKey",
				...
				resource: {
					type: "library",
					id:
						(data.serviceConnectorResourceId as string) ??
						(data.serviceConnector as any)?.resource?.id ??
						"",
					label:
						(data.serviceConnectorResourceLabel as string) ??
						(data.serviceConnector as any)?.resource?.label,
				},
				preset:
					(data.serviceConnectorPreset as string) ??
					(data.serviceConnector as any)?.preset ??
					"latestItems",
	...
	throw new Error(
		`castFeedFormDataToFeedConfig: unsupported feedType "${feedType}"`,
	);
}
