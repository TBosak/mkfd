import type {
	StructuredFeedMapping,
	JsonArrayPathCandidate,
...
export function mapStructuredDataToItems(
	data: unknown,
	...
	return getArrayByPath(data, mapping.itemPath).map((item) => {
		...
		return {
			...
			guid: stringValue(
				getByPath(item, mapping.guid) ?? getByPath(item, mapping.link),
			),
...
export function findJsonArrayPathCandidates(
	input: unknown,
): JsonArrayPathCandidate[] {
	...
	const walk = (value: unknown, path: string) => {
		if (Array.isArray(value)) {
			...
			candidates.push({
				...
				length: value.length,
				sampleKeys,
				confidence: Math.min(1, value.length / 5 + sampleKeys.length / 20),
			});
