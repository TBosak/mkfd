...
const SENSITIVE_RE =
	/password|pass|token|secret|apikey|auth|authorization|session|cookie|csrf|nonce/i;
...
export function scoreDetectedForm(form: DetectedHtmlForm): number {
	...
	if (
		!form.fields.some((field) =>
			["text", "search", "select"].includes(field.type),
		)
	)
		score -= 10;
	return score;
...
export function detectForms(html: string, pageUrl: string): DetectedHtmlForm[] {
	...
	return $("form")
		...
		.map((el, index) => {
			...
			const method = (
				($form.attr("method") || "GET").toUpperCase() === "POST"
					? "POST"
					: "GET"
			) as "GET" | "POST";
			const encoding = (
			...
			) as DetectedHtmlForm["encoding"];
			const fields = $form
				.find("input, select, textarea")
				.toArray()
				.map((input): DetectedHtmlFormField | null => {
					const $input = $(input);
			...
			const initial: DetectedHtmlForm = {
				...
				index,
				label:
					$form.attr("aria-label") || $form.attr("name") || `Form ${index + 1}`,
				method,
				actionUrl,
				encoding,
				...
				confidence: 0,
				confidenceBand: "low",
				warnings: fields.some((field) => field.type === "password")
					? ["Password fields are not recommended for automated scraping."]
					: [],
			};
...
export function detectedFormToRequestConfig(
	...
	const fields = Object.fromEntries(
		form.fields
			.filter(
				(field) =>
					!field.disabled && !["submit", "button"].includes(field.type),
			)
			.map((field) => [
				field.name,
				field.value ??
					field.options?.find((option) => option.selected)?.value ??
					"",
			]),
	);
	return {
