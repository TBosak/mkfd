...
function scanPrivateUrls(
	value: unknown,
	path = "$",
	issues: CatalogSanitizeIssue[] = [],
): CatalogSanitizeIssue[] {
	if (typeof value === "string" && /^https?:\/\//i.test(value)) {
		try {
			...
			if (isPrivateHost(url.hostname)) {
				issues.push({
					path,
					message: "Private network URLs cannot be submitted to the catalog",
					severity: "error",
				});
			}
		} catch {
		...
		value.forEach((item, index) => {
			scanPrivateUrls(item, `${path}[${index}]`, issues);
		});
...
function removePrivateFields(
	...
	removed.push({
		path: "feedId",
		reason: "Catalog entries receive a local feedId when imported",
	});
	if ("headers" in config)
		removed.push({
			path: "headers",
			reason: "Global headers may contain credentials",
		});
	if ("cookies" in config)
		removed.push({
			path: "cookies",
			reason: "Cookies are not allowed in catalog submissions",
		});
	if ("webhook" in config)
		removed.push({
			path: "webhook",
			reason: "Outgoing webhooks are local runtime behavior",
		});
	if ("flaresolverr" in config)
		removed.push({
			path: "flaresolverr",
			reason: "Solver settings are instance-specific",
		});

	const source = (
		clone.config && typeof clone.config === "object" ? clone.config : {}
	...
	for (const key of ["headers", "cookies", "request", "proxyId", "userAgent"]) {
		if (key in source) {
			...
			removed.push({
				path: `config.${key}`,
				reason:
					"Request credentials and local network settings are not catalog-safe",
			});
		}
	}
	return clone;
...
export function sanitizeForCommunityCatalog(
	...
	if (UNSUPPORTED_FEED_TYPES.has(config.feedType)) {
		errors.push({
			path: "feedType",
			message: `${config.feedType} feeds cannot be submitted to the community catalog`,
			severity: "error",
		});
	}
	...
	return {
		...
		manifestEntry: {
			...
			requiresSecrets: Boolean(
				template &&
					Object.values(template.variables).some(
						(variable) => variable.type === "secret",
					),
			),
