...
export type ResolvedProtectedValues<T> = T extends ProtectedValue
	? string
	: T extends readonly (infer Item)[]
		? ResolvedProtectedValues<Item>[]
		: T extends object
			? { [Key in keyof T]: ResolvedProtectedValues<T[Key]> }
			: T;

export function resolveProtectedValues<T>(
	input: T,
	options: { encryptionKey: string },
): ResolvedProtectedValues<T> {
	if (isProtectedValue(input)) {
		return resolveProtectedValue(
			input,
			options.encryptionKey,
		) as ResolvedProtectedValues<T>;
	}
	if (Array.isArray(input)) {
		return input.map((item) =>
			resolveProtectedValues(item, options),
		) as ResolvedProtectedValues<T>;
	}
	...
	return input as ResolvedProtectedValues<T>;
}
...
export function preserveMaskedProtectedValues<T>(incoming: T, existing: T): T {
	...
	if (Array.isArray(incoming) && Array.isArray(existing)) {
		return incoming.map((item, i) =>
			preserveMaskedProtectedValues(item, existing[i]),
		) as T;
	}
	if (
		incoming &&
