...
export async function listImapFolders(config: Config): Promise<string[]> {
  ...
  const openImapConnection = (): Promise<void> => new Promise((resolve, reject) => {
    imap.once('ready', resolve);
    imap.once('error', reject);
    imap.connect();
  });

  const getMailboxes = (): Promise<Imap.MailBoxes> => new Promise((resolve, reject) => {
  ...
  const closeConnection = (): Promise<void> => new Promise(resolve => {
    ...
    const flattenFolders = (boxes: Imap.MailBoxes, prefix = ''): string[] => {
      ...
      for (const box in boxes) {
        ...
        if (boxes[box].children) {
          folders = folders.concat(flattenFolders(boxes[box].children!, prefix + box + boxes[box].delimiter));
        }
      }
      return folders;
    };
