...
export function hasFeedTemplate(
	input: unknown,
): input is Record<string, unknown> & { template: FeedConfigTemplate } {
	return Boolean(
		input && typeof input === "object" && (input as any).template?.variables,
	);
}
...
export function findTemplateExpressions(
	...
	if (Array.isArray(input)) {
		return input.flatMap((item, index) =>
			findTemplateExpressions(item, `${path}[${index}]`),
		);
	}
	...
	for (const [name, variable] of Object.entries(template.variables ?? {})) {
		...
		if (variable.type === "url" && value) {
			try {
				...
				if (
					variable.validation?.allowedHosts &&
					!variable.validation.allowedHosts.includes(url.hostname)
				) {
					errors.push({
						path: name,
		...
		if (
			...
			value &&
			!new RegExp(variable.validation.pattern).test(String(value))
		) {
			errors.push({
				path: name,
		...
		if (variable.type === "number" && value !== undefined) {
			...
			if (
				...
				numeric < variable.validation.min
			) {
				errors.push({
					path: name,
			...
			if (
				...
				numeric > variable.validation.max
			) {
				errors.push({
					path: name,
...
function validateTemplateExpressions(
	...
	return findTemplateExpressions(input)
		.filter(
			(expr) => expr.path !== "template" && !expr.path.startsWith("template."),
		)
		.flatMap((expr) => {
...
function renderString(
	input: string,
	template: FeedConfigTemplate,
	opts: RenderFeedConfigTemplateOptions,
): unknown {
	...
	return input.replace(EXPR, (_raw, secretPrefix, name, filters) => {
		const rendered = renderExpression(
			[_raw, secretPrefix, name, filters] as any,
			template,
			opts,
		);
...
function renderExpression(
	...
	for (const filter of filters) {
		if (!ALLOWED_FILTERS.has(filter))
			throw new Error(`Template filter "${filter}" is not supported`);
	}
	let value = coerce(opts.values[name], variable);
	for (const filter of filters) value = applyFilter(value, filter);
	if (!isSecret) return value;
	const storage =
		opts.secretStorage?.[name] ?? (variable?.encrypted ? "protected" : "plain");
	if (storage === "env")
		return {
			type: "env",
			value: String(opts.values[name] ?? name),
			prefix: filters.includes("bearer") ? "Bearer " : undefined,
		};
	if (storage === "protected")
		return protectValue(String(value ?? ""), opts.encryptionKey);
	return value;
}
...
function walk(
	...
	if (Array.isArray(input))
		return input.map((item) => walk(item, template, opts));
	if (input && typeof input === "object") {
...
export function renderFeedConfigTemplate(
	...
	if (!validation.valid || expressionErrors.length > 0) {
		throw new Error(
			[...validation.errors, ...expressionErrors]
				.map((err) => err.message)
				.join("; "),
		);
	}
