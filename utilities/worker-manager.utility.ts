...
async function insertAndEmitRunLog(
	feedConfig: any,
	data: {
		startedAt: number;
	...
	try {
		...
		const row = await insertRunLog(sqlite, {
			...
			feedName:
				feedConfig.feedName ?? feedConfig.config?.title ?? feedConfig.feedId,
		...
		await pruneRunLogs(sqlite, feedConfig.feedId, {
			...
			retentionDaysEnabled: effectiveSettings.retention_days_enabled
				.value as boolean,
			retentionRuns: effectiveSettings.retention_runs.value as number,
			retentionRunsEnabled: effectiveSettings.retention_runs_enabled
				.value as boolean,
		});
	} catch (e) {
