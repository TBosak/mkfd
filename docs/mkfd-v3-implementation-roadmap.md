...
- **Typography, CF-06 — decide the type system and actually load it.** The declared fonts have never loaded (see CF-06 in the ledger), so the app has always rendered in OS fallbacks. Treat this as two separable pieces of work:
  - *The defect:* fonts must be self-hosted and bundled, never loaded from a third-party CDN. Mkfd is self-hosted and frequently runs on a LAN or air-gapped homelab, where a CDN link silently falls back to system fonts again; a Google Fonts link would also make every user's browser issue a third-party request, which has been ruled a privacy problem in EU case law. Use OFL npm packages (`geist` ships Geist and Geist Mono together; `@fontsource-variable/*` covers Inter, JetBrains Mono, IBM Plex Sans). Preload only the faces actually used, set `font-display: swap`, and replace the hardcoded `Courier New` in `FeedPreview.tsx:75`.
  - *The choice:* run `/impeccable typeset` against the real screens and let it recommend independently. **Then compare its recommendation head-to-head against Geist + Geist Mono**, which is the incumbent intent already written into `index.css` and therefore the lowest-churn option. Evaluate both at the sizes actually shipped — body text, the 11 px uppercase letterspaced workbench labels, the `workbench-value` numerals, and dense table rows — at 1280 px and 390 px. Note that JetBrains Mono is tuned for code at 13-14 px and can read as busy at label sizes, which is the specific thing to test rather than assume.
  - *Decision rights:* the maintainer makes the final call between impeccable's recommendation and Geist. Present both with real screenshots; do not land a font change on the agent's preference alone.
