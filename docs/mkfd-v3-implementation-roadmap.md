...
- Select one token system. Fix documented contrast failures, resolve animation-class collisions, add reduced-motion behavior, and either finish dark mode coherently or remove its dead surface per Packet 0. (Fonts are no longer part of this bullet: the type system was decided and self-hosted in `1d09052`, closing CF-06. See the typography item under Packet 4D.)
...
- **Build the mobile layout primitives once, against the contract 4D produces.** Today the shell is partly mobile-aware and the content is not: `BottomNav` exists with three destinations, is hidden at `lg`, and honours `env(safe-area-inset-bottom)`, but across 82 `.tsx` files the app carries only ~33 responsive utility usages and a single `@media` rule. The dense dashboard is therefore *shrunk* at 390 px rather than *adapted*. Implement the primitives here — breakpoint and density tokens, the progressive-disclosure pattern (what collapses, what defers behind a sheet or detail view, what is simply not shown on a small screen), touch-target minimums, and the navigation shell extended to cover Health and Settings — so Packets 5 through 7B build on them instead of each inventing a mobile story. This is deliberately not deferred to 9D; see Packet 4D/9D for why.
...
- **Typography — already decided and shipped; 4D validates rather than chooses.** This bullet previously described CF-06 as open and named Geist as the "incumbent intent". Both are out of date. CF-06 was closed in `1d09052`, and the maintainer's font decision is locked:
  - *Shipped today:* **Geist** for headings and titles (`--font-display`, `letter-spacing: -0.02em`), **Figtree** for body and UI controls (`--font-sans`), **Geist Mono** for code and markup previews (`--font-mono`). Self-hosted via `@fontsource-variable/{geist,geist-mono,figtree}` — 13 `@font-face` rules and 13 woff2 (173 KB) in the build, no CDN request, which is what the LAN and air-gapped case required. The tokens are mapped into Tailwind's `fontFamily`, so `font-sans`/`font-display`/`font-mono` resolve to them rather than Tailwind's defaults. `Courier New` is gone.
  - *Do not re-litigate the choice by default.* `/impeccable typeset` may still be run, but only to **validate the shipped system at the sizes actually rendered** — body copy, the 11 px uppercase letterspaced workbench labels, the `workbench-value` numerals, and dense table rows, at 1280 px and 390 px. Mono carries unusually heavy load in this UI (labels, values *and* tables), so the specific thing to test is how Geist Mono reads at 11 px uppercase, not how it reads at code sizes.
  - *Decision rights are unchanged:* the maintainer owns any change to the type system. An agent must not land a font change on its own preference, and a `typeset` recommendation that disagrees with the shipped stack is input to the maintainer, not authority to switch.
  - *Known trap for anyone revisiting this:* the `geist` npm package was tried and dropped — it ships Next.js font loaders that do not work under Vite. Use `@fontsource-variable/*`.
  - *Remaining sub-item:* the feed preview renders into a `blob:` URL document that cannot see the parent's `@font-face` rules, so it falls through to a system mono. Fixing it needs an absolute font URL and touches the iframe sandboxing Packet 2 owns; it is tracked on CF-06 and does not belong to 4D.

- **Mobile experience — decide the contract, not the CSS (maintainer request).** The desktop layout is considered good and is not the thing to change; the ask is that a phone user gets a genuinely friendlier view rather than the same information-dense dashboard at a narrower width. That is an information-architecture decision, so it is made here and built in Packet 4, not retrofitted in 9D.
  - Run `/impeccable adapt` over the shell, My Feeds, Builder, Health, Settings, and Catalog at 390 px. The skill is explicit that this is the job: *"The trap is treating adaptation as scaling. The job is rethinking the experience for the new context."* Pair it with `apple-design` for the touch, gesture, sheet, and thumb-reach questions, which are the parts `critique`/`layout` do not cover.
  - Decide and record, per surface: what stays visible, what collapses, what moves behind progressive disclosure (sheet, drawer, or a dedicated detail route), and what is deliberately desktop-only. Name the rule for each choice rather than listing pixels, so later surfaces can apply it without re-litigating.
  - Decide the density model: whether mobile gets the same components at larger touch targets, or a different summary-first composition. Dense tables and the workbench label/value pairs are the specific cases to settle, since they are where the current layout is most desktop-shaped.
  - Settle the navigation contract. `BottomNav` currently exposes three destinations and omits Health and Settings; decide whether it grows, gains an overflow affordance, or is paired with something else.
  - Output is a **mobile IA contract written into `DESIGN.md`**, plus the touch-target and reduced-motion rules. Packet 4 implements it; 9D polishes it.
...
- **Mobile polish pass**, against the contract 4D wrote and Packet 4 built: a second `/impeccable adapt` run to catch drift across the surfaces added in Packets 5 through 7B, touch-target and thumb-reach verification, sheet and gesture behaviour guided by `apple-design`, and confirmation that no interaction depends on hover. Polish only — if this pass wants to change *what is shown* on a small screen rather than how it feels, that is a 4D-contract change and should be escalated rather than absorbed here.
...
**Mobile exit criteria:** `DESIGN.md` carries the mobile IA contract and the shipped 390 px surfaces match it; every primary task — find a feed, build a feed, check health, change a setting — is completable on a phone without horizontal scrolling; no interaction requires hover; touch targets meet the agreed minimum; navigation reaches every top-level destination at every breakpoint; and the Playwright `chromium-mobile-390` project covers the adapted surfaces rather than merely asserting they render. The measure is not "it fits at 390 px" — the current layout already does that — but that a phone user is given a view shaped for a phone.
...
**Why mobile is split across 4D, Packet 4, and 9D rather than done wholly in the styling pass:** 9D deliberately runs after Packets 4 through 7B so that polish is not applied to code about to be rewritten. Deciding at 9D that the phone view needs a different information architecture would violate exactly that principle — every surface built in Packets 4, 5, 6, 7A and 7B would already exist in desktop-dense form and would have to be re-cut. The decision is cheap and the rework is expensive, so the decision moves early: 4D decides, Packet 4 builds the primitives, 9D polishes. The visual execution still belongs to the styling pass; only the contract moves forward.

**Estimated complexity:** Medium for 4D (assessment, no production changes), Medium-Large for 9D. Sequence 9D after Packet 7B and before Packet 10. The mobile contract adds roughly a slice to 4D and a slice to Packet 4; it does not change the packet ordering.
