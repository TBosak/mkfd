...
- **Tables now:** `run_logs`, `settings`, `runtime_migrations`, `feed_history_snapshots`, `feed_history_items` in one runtime DB (`runtime.db`). This is a direct rename/repoint of the existing branch-only `health.db` analytics implementation, not a second DB; use `RUNTIME_DB_PATH` and remove `DB_PATH` compatibility from this branch.
...
TIER 0 — Foundation
  ...
  SQLite Runtime Substrate           ── run_logs / settings / runtime_migrations / feed_history_snapshots / feed_history_items
