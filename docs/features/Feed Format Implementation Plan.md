...
```text
/public/feeds/{feedId}.xml   RSS 2.0
/public/feeds/{feedId}.atom  Atom
/public/feeds/{feedId}.json  JSON Feed
```
...
export async function writeAllFeedFormats(
  ...
  await Bun.write(path.join("./public/feeds", `${feedId}.xml`), outputs.rss2);
  await Bun.write(path.join("./public/feeds", `${feedId}.atom`), outputs.atom);
  await Bun.write(path.join("./public/feeds", `${feedId}.json`), outputs.json);
...
Validate `feedId` before using it in paths:

```ts
export function assertSafeFeedId(feedId: string) {
  if (!/^[A-Za-z0-9_-]+$/.test(feedId)) {
    throw new Error(`Unsafe feedId: ${feedId}`);
  }
}
...
const urls = await writeAllFeedFormats(config.feedId, feed);
```
...
const urls = await writeAllFeedFormats(feedId, feed);
```
...
```ts
const feed = buildFeedFromWhatever(...);
await writeAllFeedFormats(feedId, feed);
```
...
GET /feeds/:feedId/atom
GET /feeds/:feedId/json
```
...
writeAllFeedFormats(feedId, feed)
...
feedId validator prevents unsafe paths
```
...
| Format | URL |
|---|---|
| RSS 2.0 | `/public/feeds/{feedId}.xml` |
| Atom | `/public/feeds/{feedId}.atom` |
| JSON Feed | `/public/feeds/{feedId}.json` |
