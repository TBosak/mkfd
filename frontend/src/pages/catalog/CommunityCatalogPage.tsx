...
} from "@/components/catalog/CatalogFeedCard";
...
export function CommunityCatalogPage() {
	...
	const loadCatalog = useCallback(async () => {
		...
		try {
			...
			pushToast({
				...
				body: err instanceof Error ? err.message : "Could not load catalog",
			});
		} finally {
			setLoading(false);
		}
	...
	async function openDetail(entry: CatalogFeedEntry) {
		...
		try {
			...
			pushToast({
				...
				body: err instanceof Error ? err.message : "Catalog detail failed",
			});
		} finally {
	...
	async function importSelected(
		...
		try {
			...
			pushToast({ tone: "ok", title: "Feed imported", body: selected.title });
			setImportOpen(false);
			...
			pushToast({
				tone: "err",
				title: "Import failed",
				body: err instanceof Error ? err.message : "Could not import feed",
			});
		} finally {
			setImporting(false);
		}
				...
				<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
					<div>
						...
						<p className="text-sm text-muted-foreground">
							Browse public feed recipes and import them into this instance.
						</p>
					</div>
					<Button
						type="button"
			...
			<main className="flex-1 overflow-auto p-4 sm:p-6">
				{loading ? (
					...
					<div className="rounded-md border border-dashed p-8 text-center text-sm text-muted-foreground">
						No catalog recipes match the current filter.
					</div>
				) : (
