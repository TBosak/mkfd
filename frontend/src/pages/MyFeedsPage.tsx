...
export const MyFeedsPage: React.FC = () => {
	...
	const applyFilters = (f: FeedSummary): boolean => {
		...
		if (
			...
			return false;
		if (quickFilter === "community" && f.origin.type !== "community")
			return false;
		if (typeFilters.length > 0 && !typeFilters.includes(f.type)) return false;
		if (tagFilters.length > 0 && !tagFilters.some((t) => f.tags.includes(t)))
			return false;
		return true;
	};

	const filtered = feeds.filter(applyFilters);
	const detailFeed = detailId
		? (feeds.find((f) => f.id === detailId) ?? null)
		: null;
	const runningCount = feeds.filter(
		(f) => f.enabled && f.status !== "error" && f.status !== "disabled",
	).length;
		...
		switch (action) {
			...
			case "copy":
				navigator.clipboard.writeText(
					`${window.location.origin}/public/feeds/${feed.id}.xml`,
				);
			...
			case "duplicate": {
				try {
					const res = await fetch(`/api/feeds/${feed.id}/duplicate`, {
						method: "POST",
					});
			...
			case "enable": {
				setFeeds((prev) =>
					prev.map((f) =>
						f.id === feed.id
							? { ...f, enabled: true, status: "neverRun" as FeedStatus }
							: f,
					),
				);
				...
				if (!enableRes.ok) {
					setFeeds((prev) =>
						prev.map((f) => (f.id === feed.id ? { ...f, enabled: false } : f)),
					);
			...
			case "disable": {
				setFeeds((prev) =>
					prev.map((f) =>
						f.id === feed.id
							? { ...f, enabled: false, status: "disabled" as FeedStatus }
							: f,
					),
				);
				...
				if (!disableRes.ok) {
					setFeeds((prev) =>
						prev.map((f) => (f.id === feed.id ? { ...f, enabled: true } : f)),
					);
			...
			case "delete": {
				...
				try {
					...
					if (!res.ok) {
						...
						setFeeds(prev);
						toast.push({ tone: "err", title: "Delete failed" });
						return;
					}
				} catch {
					toast.dismiss(toastId);
			...
			<div className="flex-1 overflow-auto p-6">
				<div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-4">
					{[
						...
						<div
							key={label}
							className="workbench-panel border-t-4 p-4"
							style={{ borderTopColor: color as string }}
						>
							<div className="workbench-label">{label}</div>
							<div
								className="workbench-value"
				...
				<div className="mb-3 flex items-center justify-between">
					...
					<div style={{ display: "flex", gap: 2 }}>
						<button
							...
							style={{
								background:
									view === "cards" ? "var(--wb-primary)" : "transparent",
								color: view === "cards" ? "#fff" : undefined,
								borderColor: "var(--wb-outline)",
							}}
						...
						<button
							...
							style={{
								background:
									view === "table" ? "var(--wb-primary)" : "transparent",
								color: view === "table" ? "#fff" : undefined,
								borderColor: "var(--wb-outline)",
							}}
						...
						{QUICK_FILTERS.map(({ id, label }) => (
							<button
								...
								style={{
									...
									border: "1px solid",
									fontSize: 12,
									fontWeight: quickFilter === id ? 700 : 500,
									cursor: "pointer",
									whiteSpace: "nowrap",
									background:
										quickFilter === id ? "hsl(var(--primary))" : "transparent",
									color:
										quickFilter === id
								...
								<button
									...
									style={{
										...
										background: typeFilters.includes(type)
											? "hsl(var(--foreground))"
											: "transparent",
										color: typeFilters.includes(type)
											? "hsl(var(--background))"
											: "hsl(var(--muted-foreground))",
										borderColor: typeFilters.includes(type)
											? "hsl(var(--foreground))"
											: "hsl(var(--border))",
									}}
								>
							...
							{allTags.map((tag) => (
								<button
									...
									style={{
										...
										cursor: "pointer",
										whiteSpace: "nowrap",
										background: tagFilters.includes(tag)
											? "hsl(var(--foreground))"
											: "transparent",
