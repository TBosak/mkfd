	...
	return (
		...
		>
			...
			{hasDetails && open && (
				<div
					style={{
						borderTop: "1px solid var(--wb-outline)",
						padding: "10px 14px",
				...
				>
					...
					{rec.warnings.length > 0 && (
						<div>
							...
							>
								{rec.warnings.map((w) => (
									<li key={w.message} style={{ color: "var(--wb-warning)" }}>
										{w.message}
									</li>
								))}
							</ul>
			...
			>
				...
				<button
					...
					style={{
						padding: "5px 14px",
						borderRadius: 6,
						border: isTop ? 0 : "1px solid var(--wb-outline)",
...
export const SourceAssistantPanel: React.FC<SourceAssistantPanelProps> = ({
	onApply,
	onPickType: _onPickType,
}) => {
		...
		<div style={{ padding: "28px 24px 0", maxWidth: 980, margin: "0 auto" }}>
			...
			>
				<div style={{ padding: "16px 20px 0" }}>
					...
					>
						Paste a URL — Mkfd will check for an existing feed, sitemap,
						calendar, API, JSON-LD, and scrapeable content, then recommend the
						best path.
					</p>
					...
					{status === "idle" && (
						...
						>
							...
							>
								<input
									type="checkbox"
									checked={needsHeaders}
									onChange={(e) => setNeedsHeaders(e.target.checked)}
								/>
								...
								return (
									...
									>
										{done ? (
											<CheckCircle
												size={11}
												style={{ color: "var(--wb-success)", flexShrink: 0 }}
											/>
										) : running ? (
											<Loader2
												size={11}
				...
				{status === "error" && error && (
					...
					>
						<p style={{ margin: 0, fontSize: 13, color: "var(--wb-error)" }}>
							{error}
						</p>
						<button
							type="button"
