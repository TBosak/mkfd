...
export function SitemapForm({ register }: any) {
	return (
		<Section
			...
			title="Sitemap"
			sub="Turn sitemap URLs into feed items"
		>
			<Field label="Sitemap URL" htmlFor="sitemapUrl" required>
				<Input
					id="sitemapUrl"
					{...register("sitemapUrl", { required: true })}
			...
			<div className="grid gap-3 sm:grid-cols-2">
				<Field label="Max Items" htmlFor="sitemapMaxItems">
					<Input
						id="sitemapMaxItems"
						type="number"
						{...register("sitemapMaxItems", { valueAsNumber: true })}
						placeholder="50"
					/>
				</Field>
				<Field label="Max URLs To Scan" htmlFor="sitemapMaxUrlsToScan">
					<Input
						id="sitemapMaxUrlsToScan"
						type="number"
						{...register("sitemapMaxUrlsToScan", { valueAsNumber: true })}
						placeholder="500"
					/>
				</Field>
			</div>
