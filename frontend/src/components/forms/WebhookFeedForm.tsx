...
export function WebhookFeedForm({ register, setValue, watch }: any) {
	...
	return (
		<Section
			...
			sub="Receive JSON events and publish them as a feed"
		>
			<Field label="Slug" htmlFor="webhookSlug" required>
				<Input
					id="webhookSlug"
					{...register("webhookSlug", { required: true })}
			...
			<Field label="Bearer Token" htmlFor="webhookToken" required>
				<Input
					...
					placeholder="Leave blank to generate at save time"
				/>
			</Field>
			...
			<label
				htmlFor="webhookStoreRawPayload"
				className="flex items-center gap-2 text-sm"
			>
				<Checkbox
					id="webhookStoreRawPayload"
					checked={Boolean(watch("webhookStoreRawPayload"))}
					onCheckedChange={(v) =>
						setValue("webhookStoreRawPayload", v === true)
					}
				/>
				Store raw payload
			</label>
