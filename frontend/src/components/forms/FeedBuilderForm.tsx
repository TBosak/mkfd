...
export const FeedBuilderForm = forwardRef<
	FeedBuilderFormHandle,
	FeedBuilderFormProps
>(function FeedBuilderForm(
	{
		mode = "create",
		feedId,
		initialData,
	...
	const [feedType, setFeedType] = useState<FeedFormData["feedType"]>(
		selectedType ?? initialData?.feedType ?? "webScraping",
	);
	const [showPreview, setShowPreview] = useState(false);
	const [previewXml, setPreviewXml] = useState<string | undefined>();
	const [previewFeedConfig, setPreviewFeedConfig] = useState<
	...
	const applyWebScrapingPlan = (plan: any) => {
		...
		if (selectors.description)
			setValue("descriptionSelector", selectors.description as any);
	...
	return (
		<>
			<DraftRestoreDialog
				draft={draft}
				onRestore={() => {
					reset(draft!.data as FeedFormData);
					clearDraft();
				}}
			...
			{isSubmitting && (
				<LoadingSpinner
					message={
						mode === "edit" ? "Updating your feed..." : "Creating your feed..."
					}
					fullscreen
				/>
			)}
			...
			<form onSubmit={handleSubmit(onSubmit)} className="space-y-4 animate-in">
					...
					<Section
						...
						right={
							<span className="inline-flex items-center gap-1 rounded-md border border-border bg-muted px-2 py-1 text-xs font-medium text-muted-foreground">
								...
								{feedType === "sitemap" && <MapIcon className="h-3.5 w-3.5" />}
								{feedType === "calendar" && (
						...
						{sourceAssistantAnalysis && show("extract") && (
							<div className="space-y-2">
								...
								<WebScrapingAnalysisPanel
									jsonLdItemCount={
										sourceAssistantAnalysis?.recommendation?.webScrapingPlan
											?.jsonLd?.itemLikeCount
									}
									formCount={
										sourceAssistantAnalysis?.recommendation?.webScrapingPlan
											?.forms?.length
				...
				{feedType === "calendar" && (
					<CalendarForm register={register} setValue={setValue} watch={watch} />
				)}
				...
				{feedType === "serviceConnector" && (
					<ServiceConnectorForm register={register} />
				)}
