		...
		>
			<div className="space-y-3">
				<div className="grid gap-2 md:grid-cols-[160px_1fr_auto]">
					<Input
						value={uaName}
						onChange={(event) => setUaName(event.target.value)}
						placeholder="Profile name"
					/>
					<Input
						value={uaValue}
						onChange={(event) => setUaValue(event.target.value)}
						placeholder="User-Agent string"
					/>
					<Button type="button" variant="outline" onClick={addUserAgent}>
						Add
					</Button>
				</div>
