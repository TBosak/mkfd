		...
		},
		{
			...
			value: (
				<span style={{ fontSize: 12 }}>
					{feed.refreshMinutes != null ? `${feed.refreshMinutes} min` : "—"}
				</span>
			),
		},
		{
			...
			value: (
				<span style={{ fontSize: 12 }}>
					...
					{feed.lastNewItemCount != null && feed.lastNewItemCount > 0 && (
						<span style={{ marginLeft: 4, color: "#15803d", fontWeight: 600 }}>
	...
	return (
		...
		>
			...
			>
				...
				<button
					...
					style={{
						...
						padding: 2,
						fontSize: 16,
						color: feed.favorite ? "#f59e0b" : "hsl(var(--muted-foreground))",
					}}
			...
			<div style={{ padding: "0 14px", flex: 1 }}>
				...
				>
					{feed.sourceMethod && (
						<span style={{ marginRight: 4, fontWeight: 700, color: "#1e40af" }}>
							{feed.sourceMethod}
						</span>
					)}
					{feed.sourceUrl}
					...
					{(["rss", "atom", "json"] as FormatType[]).map((f) => (
						<button
							...
							style={{
								...
								cursor: "pointer",
								background:
									format === f ? "hsl(var(--primary))" : "transparent",
								color:
									format === f
										? "hsl(var(--primary-foreground))"
										: "hsl(var(--muted-foreground))",
								borderColor:
									format === f ? "hsl(var(--primary))" : "hsl(var(--border))",
							}}
						>
							{f.toUpperCase()}
						</button>
