...
const SENSITIVE_KEY_PATTERN =
	/authorization|cookie|x-api-key|token|secret|password|bearer/i;
...
export const KVEditor: React.FC<KVEditorProps> = ({
	...
	const updateStorage = (index: number, storage: StorageMode) => {
		const updated = rows.map((r, i) => (i === index ? { ...r, storage } : r));
		onChange(updated);
	};
	...
	const hasPlainSensitive = rows.some(
		(r) =>
			(!r.storage || r.storage === "plain") &&
			SENSITIVE_KEY_PATTERN.test(r.key) &&
			r.value.length > 0,
	);
	...
	return (
		<div>
			{hasPlainSensitive && (
				...
				>
					⚠ Some sensitive keys are stored as plain text. Consider using
					Encrypted or Env var.
				</div>
			)}
			{rows.length > 0 && (
				<table
				...
				>
					...
					<tbody>
						{rows.map((row, i) => {
							...
							return (
								<tr key={`${row.key}:${row.value}:${row.storage ?? "plain"}`}>
									...
									<td style={{ padding: "3px 4px" }}>
										<input
											...
											type={
												row.storage === "encrypted" ||
												(isSensitive && row.storage !== "plain")
													? "password"
													: "text"
											}
											style={{
